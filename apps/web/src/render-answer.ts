...
function citationBlock(cite: Citation, evidence: Map<string, EvidenceRecord>): Safe {
  ...
  return html`<details class="cite">
    <summary>
      <span class="tier">T${cite.tier}</span>${title}${cite.section_path
        ? ` | ${cite.section_path}`
        : ''}
  ...
  return html`
            ...
            ${answer.codes.map(
              (code) => html`
                <p>
                  ...
                  ${code.modifiers.length
                    ? html`${code.modifiers.map((m) => html`<span class="pill">modifier ${m}</span>`)}`
                    : ''}
                </p>
      ...
      ? html`<h2>Our experience</h2>
          <div class="card">
            ${answer.our_experience.map(
              (x) => html`<p>
                  ${x.statement}
                  ${x.numerator !== null && x.denominator !== null
                    ? html` <span class="muted">(${x.numerator} of ${x.denominator})</span>`
                    : ''}
                </p>
      ...
      ? html`<h2>Divergence</h2>
          <div class="card">
            ${answer.divergence.map(
              (d) => html`<p><strong>${d.payer_or_jurisdiction}:</strong> ${d.differs_how}</p>
    ...
    ${appeal
      ? html`<h2>Copy for appeal</h2>
          <div class="card">
            <p class="muted">Published rules only, tiers 1 to 4. Paste into your appeal letter.</p>
