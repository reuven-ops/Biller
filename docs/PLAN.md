...
## Groundedness workstream results (2026-08-25, after Phase 4)

Two targeted changes landed against gate 2's verified failure mode and were proven with live reruns (about $2.60 of model spend, all in qa_log).

1. Revise-once loop plus composer transcription discipline (prompt v2). When the verifier grades the bottom line partial or forces an abstention with concrete findings, the findings return to the composer for one revision; the revised answer is fully re-verified, fail closed, and ships only if it verifies at least as well. Live proof on A10, the item that first exposed the failure mode: the composer again stated the NCCI column order backwards, the verifier's note named the slip, and the revised answer shipped stating 90853 as column 1 and 90837 as column 2, which matches the edit table, at 90 percent verifier support ($0.36).
2. Exact-token retrieval arm (migration 0013). A1's honest abstention exposed a systemic retrieval gap: the passage that answers it (IOM 100-02 chapter 15 section 240.1.3, the AT modifier rule) ranked 14,851 of 15,968 by embedding for the question, and the english text-search config drops AT as a stopword, so no lexical query could reach it. A simple-config tsvector column with a GIN index now backs a third search arm: billing modifiers named next to the word modifier match by phrase adjacency (at next to modifier), which selects exactly the 13 modifier-defining chunks in the current corpus. Live proof: A1 flipped from abstention to a correct answer citing section 240.1.3 (AT signals active corrective treatment; claims without it are treated as maintenance and denied; initial visit must document the neuromusculoskeletal condition, the demonstrated subluxation, and the therapeutic relationship) at 89 percent verifier support.
3. Gate movement: gate 4 (abstention calibration) improves, one more must-answer item answers; gates 2 and 3 improve mechanically (higher supported rates on revised answers, expected evidence now reachable); the full gate table remeasures at the next complete eval run, ideally on the production server (Phase 6 M6.2). Gate 10 note: a revision adds one composer and one verifier pass on problem answers only; clean answers are unaffected.
