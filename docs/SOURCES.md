...
## 21. cms_remit_guides and the D14 denial code gloss references (retrieved 2026-08-25)

All URLs below were verified by live fetch on 2026-08-25 during the D14 research (four parallel passes; x12.org and wpc-edi.com were never visited). Public domain status per 17 USC 105 applies to the federal documents; the X12 origin caveat is recorded in DECISIONS.md D14.

1. IOM Pub 100-04 chapter 22, Remittance Advice: https://www.cms.gov/regulations-and-guidance/guidance/manuals/downloads/clm104c22.pdf (HTTP 200, 20 pages, Rev. 11427 issued 05-20-22). Group code definitions in 60.1, CARC framework 60.2, RARC framework 60.3. Added to the cms_iom chapter list; ingested with this change.
2. MLN booklet ICN905367, Remittance Advice Resources and FAQs: https://www.cms.gov/outreach-and-education/medicare-learning-network-mln/mlnproducts/downloads/icn905367textonly.pdf (HTTP 200, 15 pages). CMS authored explainer of ERA and SPR, group codes, CARC and RARC roles, and PLB codes; no third party copyright notices in the text-only edition. Queued in cms_remit_guides (Phase 4).
3. CCIIO guidance, Required RARCs under the CAA and the No Surprises Act: https://www.cms.gov/cciio/programs-and-initiatives/other-insurance-protections/caa-nsa-rarc-codes.pdf (HTTP 200, 7 pages, July 2026). Lists NSA RARCs N871 to N877 with text; footnote 5 states the descriptions are reproduced with X12 and WPC permission for that prescribed purpose, the key licensing signal behind D14. Queued in cms_remit_guides (Phase 4).
4. CMS remittance advice overview page: https://www.cms.gov/medicare/coding-billing/electronic-billing/health-care-payment-remittance-advice (HTTP 200; cms.gov serves 403 to some fetch tools, use curl). Plain language group code meaning and the free MREP and PC Print viewers.
5. Quarterly update cadence: the recurring CARC, RARC, MREP, PC Print update series shows lists change about three times per year. Modern installments carry no code text (verified MM12102 2021, MM12774 2022, R13482CP 2025, R13791CP 2026); historical installments do (verified R2372CP 2011 with full CARC tables, MM6229 2008 with RARC tables) and serve as dated drafting references only.
6. State Medicaid crosswalks, drafting references only, not ingested: Indiana IHCP https://secure.in.gov/medicaid/partners/files/Edit-Audit-EOB-ARC-Crosswalk.xlsx (about 62 CARCs and 103 RARCs verbatim); MassHealth https://www.mass.gov/info-details/835-payment-advice-and-eobcarc-rarc-lists (XLSX posted 2026-02-12, about 24 CARCs and 193 RARCs, some rows state paraphrases). State works are outside 17 USC 105 and Mass.gov terms flag embedded third party copyright material.
7. CAQH CORE code combinations: REJECTED under hard rule 11. Current version v3.10.0 February 2026 lives on https://www.dataspring.com/core/operating-rules (CAQH now does business as DataSpring); the spreadsheet embeds the full X12 description text, and the DataSpring terms of service (effective 2026-06-08) prohibit copying, reproduction, and derivative works of site content without written consent. Not downloaded into the corpus; no courier exists.
8. MREP software https://www.cms.gov/data-research/cms-information-technology/access-cms-data-application/medicare-remit-easy-print: distributed free for providers to view their own remits; extracting its embedded code list for a different product exceeds that purpose and is not done.

## 22. payer_policies (payer policy libraries), discovery record (retrieved 2026-08-25)

Seven parallel verification passes on 2026-08-25, all by live fetch with curl, no logins, no click-through agreements accepted. Every URL below returned HTTP 200 on the retrieval date. The controlling finding: each commercial library is public and robots-permitted, but site terms of use prohibit automated collection, so the source runs as lead uploads (kind upload in config/sources.yaml); see DECISIONS.md D16 for the decision and the verbatim clauses.

1. UnitedHealthcare: commercial reimbursement policy index https://www.uhcprovider.com/en/policies-protocols/commercial-policies/commercial-reimbursement-policies.html (roughly 100 policy PDFs with titles, plus monthly Reimbursement Policy Update Bulletins). Sample verified: Physical Medicine and Rehabilitation Maximum Combined Frequency per Day Policy 2026R0101A, PDF, dated History section. robots.txt allows the paths. Terms of use on uhc.com prohibit scraping and harvesting; blocked for automation per D16.
2. Aetna: Clinical Policy Bulletins A to Z index https://www.aetna.com/cpb/medical/data/cpb_alpha.html (629 medical CPBs; per-policy pages carry Effective, Last Review, Next Review dates). Sample verified: CPB 0107 Chiropractic Services, effective 03/25/1995, last review 04/07/2026. robots.txt allows the static index. Terms prohibit systematic downloading and robots without written consent; blocked for automation per D16. CPBs embed CPT under Aetna's AMA license; CPT_LICENSE_MODE=none applies to any upload.
3. Cigna: coverage policy library https://static.cigna.com/assets/chcp/resourceLibrary/coveragePolicies/index.html with per-letter A to Z tables carrying per-policy effective dates; policy update feed at latestUpdatesListing.html. Sample verified: CPG 024 Acupuncture PDF, Effective Date 4/15/2026 on page 1. robots.txt allows the library paths. The cigna.com terms of use page could not be read (its content script's JSON returns 404); unreadable terms are not permission, so blocked for automation per D16. Note: chiro, PT, and acupuncture clinical guidelines are delegated to ASH (ashlink.com) and are not in this library.
4. Elevance/Anthem: clinical guidelines and medical policies at https://www.anthem.com/provider/policies/clinical-guidelines/ with a machine-readable JSON index at https://www.anthem.com/api/getmedicalpolicies/?state=oh (450 entries for Ohio: titles with policy numbers, dates, status, HTML document links under /dam/medpolicies/). Sample verified: CG-OR-PR-09, publish date 10/01/2025, document history table. Reimbursement policies live separately at /provider/individual-commercial/reimbursement. robots.txt allows these paths. Terms of use prohibit robots and systematic database building; blocked for automation per D16.
5. Humana: claims payment policies index https://provider.humana.com/coverage-claims/claims-payment-policies backed by the public JSON API https://provider.humana.com/api/forms/claimPaymentPolicy (114 policies with titles, descriptions, published dates, tokenized PDF links). Sample verified: CP2024014 Chiropractic Services for Acute Treatment, Medicare Advantage, published 12/2024. The index host allows all robots; the PDF host dctm.humana.com disallows all robots and each PDF bars storage in a retrieval system without written permission; blocked for automation per D16, and see the D16 counsel flag on uploads. A second library (mcp.humana.com medical and pharmacy coverage policies) is browsable with effective dates but gates each document behind a click-through; treated as portal-equivalent.
6. Florida Blue: medical coverage guidelines at https://mcgs.bcbsfl.com/Default (public index of about 1,500 titles by category; per-policy PDFs by stateless GET with the imgId embedded in the index). Sample verified: MCG 01-97000-01 Physical Therapy and Occupational Therapy, reviewed 02/26/26, revised 03/15/26. No robots.txt on the subdomain. floridablue.com terms prohibit scrapers and copying; blocked for automation per D16.
7. Florida Medicaid (AHCA): adopted rules, service-specific coverage policies at https://ahca.myflorida.com/medicaid/rules/adopted-rules-service-specific-policies.html (HTML table of rule number, name, effective date, with direct PDF links; about 150 entries including Chiropractic 59G-4.040, Physical Therapy, and the behavioral health 59G-4 series; reimbursement schedules under rule 59G-4.002). Sample verified: 59G-4.040 Chiropractic Services PDF, effective 2/11/2019. No site terms page exists; robots.txt allows all for generic agents with content signal search=yes, ai-train=no, use=reference. Permissible for an automated courier under D16 item 4 (retrieval and citation only, honest user agent, no training use); queued as a fast follow, disabled until the payer list is confirmed.

## 23. client_contracts, payer_call_notes, remit_behavior (internal sources, active 2026-08-25)

These three sources hold ClinicMind's own material, so there is nothing to discover externally. client_contracts: lead uploads on the Admin page, chunked at tier 5 with client_id isolation, machine-readable fee schedules also parsed into client_fee_schedule. payer_call_notes: the Appendix C form on the Notes page, PHI-screened, tier 6, with approval and expiry workflow. remit_behavior: the Appendix B CSV importer on the Admin page, aggregates only, raw files never stored. All three are enabled in config/sources.yaml with cadence 0 (event-driven, no courier).

## Sources 14 and 24 (remaining)

mac_sites (Phase 4, pending the MAC and state footprint from Reuven) and ama_cpt (Phase 7, pending the AMA license amendment) get discovery entries when that work starts.
